export const ERROR_VISIT_DATE = {
  DATE_NOT_A_POSITIVE_INTEGER:
    '[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.',
  DATE_OUT_OF_BOUND: '[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.',
};

export const ERROR_ORDER = {
  QUANTITY_NOT_POSITIVE_INTEGER:
    '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.',
  MENU_NOT_EXISTING: '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.',
};

export const ERROR_ORDERLIST = {
  MENU_NOT_UNIQUE: '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.',
  MENU_ONLY_DRINKS: '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.',
  QUANTITY_TOO_MUCH: '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.',
  ELEMENT_NOT_ORDER: '[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.',
};

export const ERROR_CALENDAR = {
  MONTH_NOT_A_POSITIVE_INTEGER: '[ERROR] 유효하지 않는 달력입니다.',
  YEAR_NOT_A_POSITIVE_INTEGER: '[ERROR] 유효하지 않는 연도입니다.',
  MONTH_OUT_OF_BOUND: '[ERROR] 유효하지 않는 월입니다.',
};

export const ERROR_INSTANCE = {
  ERROR_ORDERLIST_NOT_MADE_OF_ORDER: `[ERROR] Elements not made of Order Instance`,
  ERROR_DISCOUNTLIST_NOT_MADE_OF_BENEFIT: `[ERROR] Elements not made of Benefit Instance`,
  ERROR_NOT_VISIT_DATE: `[ERROR] Instance not VisitDate`,
  ERROR_NOT_ORDERLIST: `[ERROR] Instance not OrderList`,
  ERROR_NOT_FREEBIE_BENEFIT: `[ERROR] Instance not Freebie Benefit`,
};
