...
import { ERROR_MENU } from '../constants/ErrorMessage.js';

export const isExistingMenu = (menuList) => {
  menuList.forEach((menu) => {
    if (!(menu in WOOWA_MENU)) throw new Error(ERROR_MENU.MENU_NOT_EXISTING);
  });
};
...
export const isUniqueMenuList = (menuList) => {
  ...
  if (deleteDuplicateMenu.size !== menuList.length)
    throw new Error(ERROR_MENU.MENU_NOT_UNIQUE);
};
...
export const isNotOnlyDrink = (menuList) => {
  ...
  if (!checkNotOnlyDrink) throw new Error(ERROR_MENU.MENU_ONLY_DRINKS);
};
